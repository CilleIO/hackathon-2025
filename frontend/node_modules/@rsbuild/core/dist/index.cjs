...
${section.body}` : section.body).join("\n\n"));
        ...
        if (showDetail) {
            ...
            if (showTotal) {
                ...
                if (customTotal) logs.push(customTotal);
                    ...
                    let log = '';
                    if (log += ' '.repeat(maxFileLength1 - totalSizeLabel.length), log += color.magenta(totalSizeLabel), log += `   ${totalSizeStr}`, options.compressed) {
    ...
    let CACHE_CONTROL_NO_CACHE_REGEXP = /(?:^|,)\s*?no-cache\s*?(?:,|$)/, statuses = {
        ...
        let outputFileSystem1 = function(writeToDisk, compilers) {
            if (!0 !== writeToDisk) {
            ...
            ready(async function() {
                ...
                if (rangeHeader) {
                    ...
                    if (-2 === parsedRanges ? logger.error("[rsbuild:middleware] A malformed 'Range' header was provided. A regular response will be sent for this request.") : parsedRanges.length > 1 && logger.error("[rsbuild:middleware] A 'Range' header with multiple ranges was provided. Multiple ranges are not supported, so a regular response will be sent for this request."), -2 !== parsedRanges && 1 === parsedRanges.length) {
                        let offset;
        ...
        sendError(errors, token) {
            ...
            this.sockWrite({
                ...
                data: {
                    ...
                    html: (errors1 = formattedErrors, root = this.context.rootPath, htmlItems = errors1.map((item)=>{
...
`)
        ...
        sendStats({ force = !1, token }) {
            ...
            let initialChunks = this.initialChunksMap.get(token), shouldReload = stats.entrypoints && initialChunks && (a = initialChunks, b = newInitialChunks, !(a.size === b.size && [
                ...a
...
[rsbuild] initConfigs() can only be called with the same action type.
  ...
  - Actual: ${options?.action}`);
    ...
    async function createRsbuild(options = {}) {
        ...
        let plugins, context1, context2, envs = options.loadEnv ? loadEnv({
            cwd: options.cwd,
...
`)
        ...
        }), context3 = await createContext(resolvedOptions, config), getPluginAPI = function({ context, pluginManager }) {
        ...
        context3.getPluginAPI = getPluginAPI;
        let globalPluginAPI = getPluginAPI();
        logger.debug('registering default plugins'), pluginManager = pluginManager1, context = context3, pluginManager.addPlugins([
            {
                name: 'rsbuild:basic',
                setup (api) {
            ...
            (context1 = context, {
                ...
                setup (api) {
                    api.onAfterBuild(async ({ stats, environments, isFirstCompile })=>{
                        if (!stats || context1.buildState.hasErrors || !isFirstCompile) return;
                        let logs = [];
...
`;
            ...
            },
            {
                ...
                setup (api) {
                    ...
                    api.modifyBundlerChain((chain, { CHAIN_ID, environment, isDev })=>{
                        ...
                        }(manifest), { RspackManifestPlugin } = requireCompiledPackage('rspack-manifest-plugin'), { htmlPaths: htmlPaths1 } = environment, filter = manifestOptions1.filter ?? ((file)=>!file.name.endsWith('.LICENSE.txt'));
        ...
        let provider = config.provider || rspackProvider, providerInstance = await provider({
            ...
            context3.action = 'preview', process.env.NODE_ENV || setNodeEnv('production');
            let config = await initRsbuildConfig({
                context: context3,
                pluginManager: pluginManager1
            }), { distPath } = context3, { checkDistDir = !0 } = options;
            if (checkDistDir) {
                var path;
                let files;
                if (!(0, external_node_fs_namespaceObject.existsSync)(distPath)) throw Error(`${color.dim('[rsbuild:preview]')} The output directory ${color.yellow(distPath)} does not exist, please build the project before previewing.`);
                if (path = distPath, 0 === (files = external_node_fs_default().readdirSync(path)).length || 1 === files.length && '.git' === files[0]) throw Error(`${color.dim('[rsbuild:preview]')} The output directory ${color.yellow(distPath)} is empty, please build the project before previewing.`);
            }
            return startProdServer(context3, config, options);
        }, build = async (...args)=>{
            context3.action = 'build', process.env.NODE_ENV || setNodeEnv('production');
            let buildInstance = await providerInstance.build(...args);
            return {
                ...buildInstance,
                close: async ()=>{
                    await context3.hooks.onCloseBuild.callBatch(), await buildInstance.close();
                }
            };
        }, initAction = ()=>{
            context3.action || (context3.action = 'development' === config.mode ? 'dev' : 'build');
        }, rsbuild = {
            build,
            preview,
            startDevServer: (...args)=>(context3.action = 'dev', process.env.NODE_ENV || setNodeEnv('development'), providerInstance.startDevServer(...args)),
            createCompiler: (...args)=>(initAction(), providerInstance.createCompiler(...args)),
            createDevServer: (...args)=>(context3.action = 'dev', process.env.NODE_ENV || setNodeEnv('development'), providerInstance.createDevServer(...args)),
        ...
        return config.environments && await Promise.all(Object.entries(config.environments).map(async ([name, environmentConfig])=>{
            if (!environmentConfig.plugins || context3.specifiedEnvironments && !context3.specifiedEnvironments.includes(name)) return;
